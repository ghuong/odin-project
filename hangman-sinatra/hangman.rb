...
class HangmanGameState
  ...
  def initialize(game_id = 0, word = nil, correct_letter_guesses = nil, incorrect_letter_guesses = nil)
  ...
  def display_game_status
    status = ""
    status << display_word_status

    if not @incorrect_letter_guesses.empty?
      status << "Wrong Letters: " << @incorrect_letter_guesses.join(" ")
    end

    remaining_life = get_life_remaining
    status << " (" << remaining_life.to_s
    if remaining_life > 1 
      status << " Lives Left)"
    else
      status << " Life Left!)"
